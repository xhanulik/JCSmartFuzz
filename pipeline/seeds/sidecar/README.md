...
* `--model` — model name/alias; overrides the default from env/`llm_config.ini`
* `LLM_API_TOKEN` env var — Bearer token

## Usage Examples

All commands must be run from the `pipeline/seeds/` directory so that `sidecar` is on the Python path:
```bash
cd pipeline/seeds
```

**List available models on the configured LLM endpoint:**
```bash
export LLM_API_TOKEN=<your-token>
python3 -m sidecar --list-models
```

**Basic run with the configured default model:**
```bash
export LLM_API_TOKEN=<your-token>
python3 -m sidecar \
...
Edit that file to tune the wording or generation guidance — no code change
needed. `{{marker}}` placeholders (e.g. `{{source_context}}`,
`{{fuzzer_state_section}}`, `{{acceptance_line}}`) are filled by
